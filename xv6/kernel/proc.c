...
// maximum ticks from q0 to q3
int max_t[4] = {16,8,4,2};
...
scheduler(void)
{
  ...
  for(;;){
	...
	for (int i = 0; i <= i3; i++) {	
	    ...
	    if(p->ticks >= max_t[3] && p->state != SLEEPING) {
		i2++;
		if (p->prio > 0) {
		    p->prio--;
		}
    ...
    if(i2 != -1) {
	    ...
	    if(p->ticks >= max_t[2] && p->state != SLEEPING) {
		i1++;
		if(p->prio > 0){
		    p->prio--;
		}
    ...
    if(i1 != -1) {
	    ...
	    if(p->ticks >= max_t[1] && p->state != SLEEPING) {
		i0++;
		if(p->prio > 0) {
		    p->prio--;
		}
...
sleep(void *chan, struct spinlock *lk)
{
  ...
  //increment block and switch into scheduler
  sched();
...
procinfo(uint64 proc_array, int max_procs)
{
  ...
  for (cur_proc=proc; cur_proc < &proc[NPROC]; cur_proc++)
  {
    ...
    cur_uproc.block = cur_proc->block;
    cur_uproc.full = cur_proc->full;
